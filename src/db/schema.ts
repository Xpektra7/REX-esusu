import {
  pgTable, uuid, varchar, integer, boolean, timestamp, jsonb, text, uniqueIndex, index,
...
export const circles = pgTable("circles", {
  ...
  defaultResolutionRule: varchar("default_resolution_rule", { length: 20 }).default("absorb").notNull(),
...
export const membersCircles = pgTable("members_circles", {
  id: uuid("id").defaultRandom().primaryKey(),
  ...
  index("idx_members_user_circle_status").on(table.userId, table.circleId, table.status),
]);
...
export const cycles = pgTable("cycles", {
  id: uuid("id").defaultRandom().primaryKey(),
  circleId: uuid("circle_id").notNull().references(() => circles.id),
  recipientMemberId: uuid("recipient_member_id").notNull().references(() => membersCircles.id),
...
export const contributions = pgTable("contributions", {
  id: uuid("id").defaultRandom().primaryKey(),
  memberCircleId: uuid("member_circle_id").notNull().references(() => membersCircles.id),
  ...
  nombaTransactionRef: varchar("nomba_transaction_ref", { length: 255 }).unique(),
  ourReference: varchar("our_reference", { length: 255 }).unique(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  reconciledAt: timestamp("reconciled_at"),
}, (table) => [
  index("idx_contributions_member_cycle").on(table.memberCircleId, table.cycleId),
...
export const debts = pgTable("debts", {
  id: uuid("id").defaultRandom().primaryKey(),
  cycleId: uuid("cycle_id").notNull().references(() => cycles.id),
  debtorMemberId: uuid("debtor_member_id").notNull().references(() => membersCircles.id),
  creditorMemberId: uuid("creditor_member_id").notNull().references(() => membersCircles.id),
  amountKobo: integer("amount_kobo").notNull(),
...
export const webhookEvents = pgTable("webhook_events", {
  id: uuid("id").defaultRandom().primaryKey(),
  nombaRequestId: varchar("nomba_request_id", { length: 255 }).unique().notNull(),
  ...
  index("idx_webhooks_nomba_request_id").on(table.nombaRequestId),
]);
...
export const inviteCodes = pgTable("invite_codes", {
  ...
  index("idx_invite_codes_circle").on(table.circleId),
]);
