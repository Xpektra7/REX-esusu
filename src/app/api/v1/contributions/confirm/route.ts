...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    }).from(contributions).where(eq(contributions.ourReference, ourReference)).limit(1);
    if (!contribution) return error("Contribution not found", "04", 404);

    const [mc] = await db.select().from(membersCircles).where(eq(membersCircles.id, contribution.memberCircleId)).limit(1);
    if (!mc || mc.userId !== auth.user!.userId) return error("Not your contribution", "03", 403);
    ...
    return success({ status: "pending_reconciliation" }, "Awaiting confirmation. Funds will reflect shortly.");
  } catch (e) {
