...
import { db } from "@/db";
import { circles, membersCircles, cycles, users } from "@/db/schema";
...
export async function POST(
  ...
  try {
    ...
    if (circle.status !== "pending") return error("Circle already active or completed");
    ...
    const [membership] = await db.select().from(membersCircles)
      ...
      .limit(1);
    if (!membership || membership.role !== "admin") return error("Only admin can activate circle", "03", 403);

    const [memberCount] = await db.select({ count: count() }).from(membersCircles)
      .where(eq(membersCircles.circleId, id));
    if (Number(memberCount.count) < 2) return error("Need at least 2 members to activate circle");
    ...
    for (let i = 0; i < members.length; i++) {
      if (!members[i].rotationOrder) {
        await db.update(membersCircles)
    ...
    const [cycle] = await db.insert(cycles).values({
      ...
      cycleNumber: 1,
      expectedTotalKobo: circle.contributionAmountKobo * Number(memberCount.count),
      actualTotalKobo: 0,
      status: "active",
      startsAt: new Date(),
      deadlineAt: new Date(Date.now() + circle.gracePeriodHours * 60 * 60 * 1000),
    }).returning();
