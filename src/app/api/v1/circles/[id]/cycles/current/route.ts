...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [recipient] = await db.select({
      ...
      .where(eq(membersCircles.id, currentCycle.recipientMemberId)).limit(1);
    ...
    const memberRows = await db.select({
      ...
      .where(and(eq(membersCircles.circleId, id), eq(membersCircles.status, "active")));
    ...
    }).from(contributions).where(eq(contributions.cycleId, currentCycle.id));
    ...
    for (const m of memberRows) {
      ...
      memberContributions.push({
        ...
        status: totalPaid >= circle.contributionAmountKobo ? "fully_applied" : totalPaid > 0 ? "partial" : "pending",
    ...
    return success({
      ...
      recipient: recipient ? { id: recipient.userId, name: recipient.name } : null,
