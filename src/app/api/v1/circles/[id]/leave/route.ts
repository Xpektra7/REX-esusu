...
import { db } from "@/db";
import { circles, membersCircles, debts } from "@/db/schema";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const activeDebts = await db.select().from(debts)
      .where(and(eq(debts.debtorMemberId, membership.id), eq(debts.status, "active")))
      .limit(1);
    if (activeDebts.length > 0) {
      return error("Cannot leave circle with active debts. Clear your debts first.", "07");
    }

    if (membership.role === "admin") {
      const otherAdmin = await db.select().from(membersCircles)
        .where(and(
          eq(membersCircles.circleId, id),
          eq(membersCircles.role, "admin"),
          eq(membersCircles.status, "active"),
        )).limit(1);
      if (!otherAdmin || (otherAdmin.length === 1 && otherAdmin[0].userId === auth.user!.userId)) {
        return error("Admin cannot leave without transferring admin role to another active member");
      }
    }
